...
class VGGNet(Ciresan2012Column):
    def __init__(self, data_stream, batch_size, learning_rate, momentum, leakiness, model_spec, pad=1, params=None):
...
def train_drnet(network, learning_rate, momentum, n_epochs, dataset,
                 batch_size, leakiness, center, normalize):
...
if __name__ == '__main__':
    ...
    dataset = arg.get('dataset') or "data/train/centered_crop/"
    ...
    train_drnet(network=network, learning_rate=learning_rate, momentum=momentum, n_epochs=n_epochs, dataset=dataset, batch_size=batch_size, leakiness=leakiness, center=center, normalize=normalize)
